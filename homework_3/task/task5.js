...
function luckyNumber(number){
    if (isluckyNumber(number) && almostLuckyNumber(number)){
    return true;
    } return false;
}
...
function isluckyNumber(number){
    ...
    for (var i = 0; i <number.length; i++){
        if(number[i] === '4' || number[i]=== '7') countNumber++;
    } if (countNumber === Number(number.length)){
        return true;
    } else return false;
}


function almostLuckyNumber(number) {
    var digits = number.toString().length;
    return isluckyNumber(digits);
}
